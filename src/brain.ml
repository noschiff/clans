(*
* @Author: UnsignedByte
* @Date:   2022-03-24 17:24:35
* @Last Modified by:   UnsignedByte
* @Last Modified time: 2022-03-24 21:07:26
*)
...
(** Uses box-muller transform to generate normal
		distributed random numbers *)
let normal () = 
	let x, y = Random.float 1., Random.float 1. in
	sqrt (-2. *. log x) *. cos (2. *. Float.pi *. y)

(** Creates an mxn matrix with normal random
		dostributed numbers *)
let normal_matrix m n =
	...
	let rec f a = function
		| 0 -> []
		| b -> g a :: f a (b-1)
	in f m n |> Matrix.of_list

let create i o m l =
	let ls = (i+m) :: l @ [o+m] in
	{
		weights = (let rec f a b l = 
			normal_matrix a b :: match l with
				| [] -> []
				| c :: l2 -> f b c l2
		in ls
			|> (function
